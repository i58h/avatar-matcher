...
def get_harmony_colors(
    ...
    """
    استخراج أبرز الألوان المتناسقة بين الأفاتار والبنر.
    الإرجاع: قائمة HEX strings — لو استخدمتها على البروفايل أو البنر تطلع متناسقة.
    """
    scored = []

    for a_color, a_weight in avatar_palette:
        for b_color, b_weight in banner_palette:
            score, _ = smart_color_match(a_color, b_color)

            if score >= 60:
                scored.append((score * a_weight, a_color))
                scored.append((score * b_weight, b_color))

    # لو ما في تناسق كافٍ، نرجع أبرز ألوان البنر
    if not scored:
        return [_to_hex(c) for c, _ in
                sorted(banner_palette, key=lambda x: x[1], reverse=True)[:top_n]]

    # ترتيب وإزالة المكررات
    scored.sort(key=lambda x: x[0], reverse=True)
    seen, result = set(), []
    for _, color in scored:
        hex_c = _to_hex(color)
